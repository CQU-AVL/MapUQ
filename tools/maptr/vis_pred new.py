...
def project_and_draw_on_image(img_path, proj_mat, pts_xy, color=(0, 0, 255), thickness=2):
    """
    proj_mat: (3,4) numpy array mapping lidar (homogeneous) to image pixels
    pts_xy: (N,2) numpy array in lidar/BEV coordinates (x,y). z assumed 0.
    """
    try:
        ...
        z = np.zeros_like(x)
        ones = np.ones_like(x)
        # cam_coords shape (4, N)
        cam_coords = np.vstack([x, y, z, ones])
        pix = perspective(cam_coords, proj_mat)  # returns (M,2)
        if pix.shape[0] < 2:
            return False
        # draw polyline
        pts_int = np.round(pix).astype(int)
        for i in range(len(pts_int) - 1):
            p1 = tuple(pts_int[i].tolist())
            p2 = tuple(pts_int[i + 1].tolist())
            cv2.line(img, p1, p2, color, thickness, lineType=cv2.LINE_AA)
        # draw points
        for p in pts_int:
            cv2.circle(img, tuple(p.tolist()), radius=1, color=color, thickness=-1)
        out_path = img_path.replace('.jpg', '_proj.jpg')
...
def main():
    ...
    for i, data in enumerate(data_loader):
        ...
        # Project predicted lane points onto each camera image and save overlay
        try:
            # Project GT lane points as well (if available)
            try:
                gt_fixed_pts = gt_bboxes_3d[0].fixed_num_sampled_points
                if hasattr(gt_fixed_pts, 'cpu'):
                    gt_fixed_pts_arrs = [g.cpu().numpy() for g in gt_fixed_pts]
                else:
                    gt_fixed_pts_arrs = [g for g in gt_fixed_pts]
                for cam_name, img_path in img_path_dict.items():
                    if cam_name in CAMS:
                        cam_idx = CAMS.index(cam_name)
                    else:
                        continue
                    proj_mat = None
                    if isinstance(img_metas, (list, tuple)) and cam_idx < len(img_metas) and 'lidar2img' in img_metas[cam_idx]:
                        proj_mat = np.array(img_metas[cam_idx]['lidar2img'])
                    elif 'lidar2img' in img_metas:
                        try:
                            proj_mat = np.array(img_metas['lidar2img'][cam_idx])
                        except Exception:
                            proj_mat = None
                    elif isinstance(img_metas, (list, tuple)) and 'lidar2img' in img_metas[0]:
                        try:
                            proj_mat = np.array(img_metas[0]['lidar2img'][cam_idx])
                        except Exception:
                            proj_mat = None
                    if proj_mat is None:
                        continue
                    for single_gt in gt_fixed_pts_arrs:
                        try:
                            pts_xy = single_gt.reshape(-1, 2)
                        except Exception:
                            continue
                        project_and_draw_on_image(img_path, proj_mat, pts_xy, color=(0, 255, 0), thickness=2)
            except Exception:
                pass

            result_dic_tmp = result[0]['pts_bbox']
            pred_pts_all = result_dic_tmp.get('pts_3d', None)  # Tensor or ndarray
            if pred_pts_all is not None:
                # ensure numpy
                if hasattr(pred_pts_all, 'cpu'):
                    pred_pts_all = pred_pts_all.cpu().numpy()
                # iterate cameras
                for cam_name, img_path in img_path_dict.items():
                    # find cam index
                    if cam_name in CAMS:
                        ...
                        # try integer suffix mapping
                        continue
                    proj_mat = None
                    # try several possible metadata locations for lidar2img
                    if isinstance(img_metas, (list, tuple)) and cam_idx < len(img_metas) and 'lidar2img' in img_metas[cam_idx]:
                        proj_mat = np.array(img_metas[cam_idx]['lidar2img'])
                    elif 'lidar2img' in img_metas:
                        try:
                    ...
                    if proj_mat is None:
                        continue
                    # pred_pts_all shape: (num_preds, num_pts_per_vec, 2) or (num_preds, num_pts, 2)
                    # We'll flatten and draw each prediction polyline
                    for single_pred in pred_pts_all:
                        # single_pred may be (num_pts,2)
                        try:
                            pts_xy = single_pred.reshape(-1, 2)
                        except Exception:
                            continue
                        project_and_draw_on_image(img_path, proj_mat, pts_xy, color=(0, 0, 255), thickness=2)
        except Exception:
            pass
