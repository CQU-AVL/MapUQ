...
model = dict(
    ...
    pretrained=dict(img='/home/ubuntunew/model/MapTR-maptrvnew/maptrv2_nusc_r50_24e.pth'),
    img_backbone=dict(
        type='ResNet',
        depth=50,
        num_stages=4,
    ...
    pts_bbox_head=dict(
        ...
        aux_seg=aux_seg_cfg,
        # z_cfg=z_cfg,
        # Lane uncertainty configuration
        lane_uncertainty_config=dict(
            use_lane_uncertainty=True,
            mc_forward_num=5,
            dropout_rate=0.1,
        ...
        # ROI uncertainty configuration (progressive ROI scale adjustment)
        roi_uncertainty_cfg=dict(
            use_roi_uncertainty=True,
            init_scale=1.0,
            adapter_cfg=dict(
                alpha=0.25,
                min_scale=0.3,
            ...
            refine_with_roi_scales=False,  # Set to True for training-time refinement
        ),
        roi_scale_loss_cfg=dict(
            weight=1.0,
            reduction='mean'
        ),
        # Segmentation uncertainty configuration
        segmentation_uncertainty_cfg=dict(
            use_segmentation_uncertainty=True,
            bev_seg_cfg=dict(
                mc_forward_num=5,
                dropout_rate=0.3,
                temperature=1.0,
...
data_root = '/home/ubuntunew/model/nuscences-mini/data/nuscenes/'
