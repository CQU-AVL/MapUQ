...
class MapTRv2Head(DETRHead):
    ...
    def __init__(self,
                ...
                #  num_vec=20,
                 ...
                 lane_uncertainty_config=None,
                 roi_uncertainty_cfg=None,
                 roi_scale_loss_cfg=None,
                 segmentation_uncertainty_cfg=None,
                 **kwargs):
        ...
        # Segmentation uncertainty configuration (must be set before super().__init__ triggers _init_layers)
        self.segmentation_uncertainty_cfg = segmentation_uncertainty_cfg or {}
        self.use_segmentation_uncertainty = self.segmentation_uncertainty_cfg.get('use_segmentation_uncertainty', False)
        ...
        # Initialize lane uncertainty modules
        self.lane_uncertainty_config = lane_uncertainty_config
        if lane_uncertainty_config is not None and lane_uncertainty_config.get('use_lane_uncertainty', False):
            from .lane_uncertainty_heads import (
                UncertaintyClassificationHead, UncertaintySampleExtractor)
            from ..losses.lane_uncertainty_losses import LaneUncertaintyLoss

            # Only keep parameters supported by each component to avoid unexpected kwargs
            head_allowed_keys = {'mc_forward_num', 'dropout_rate', 'uncertainty_threshold'}
            head_kwargs = {k: v for k, v in lane_uncertainty_config.items()
                           if k in head_allowed_keys}

            # Initialize uncertainty classification head for lane detection
            self.lane_uncertainty_head = UncertaintyClassificationHead(
                input_dim=self.embed_dims,
                num_classes=self.cls_out_channels,
                **head_kwargs
            )

            extractor_allowed_keys = {'mc_forward_num', 'confidence_threshold', 'uncertainty_ratio'}
            extractor_kwargs = {k: v for k, v in lane_uncertainty_config.items()
                                if k in extractor_allowed_keys}

            # Initialize uncertainty sample extractor
            self.lane_uncertainty_extractor = UncertaintySampleExtractor(
                **extractor_kwargs
            )

            loss_kwargs = {k: v for k, v in lane_uncertainty_config.items()
                           if k not in ['use_lane_uncertainty'] + list(head_allowed_keys) + list(extractor_allowed_keys)}

            # Initialize lane uncertainty loss
            self.lane_uncertainty_loss_fn = LaneUncertaintyLoss(
                **loss_kwargs
            )

            # Projection to match lane_uncertainty_head input dimension
            feature_dim = self.cls_out_channels + self.num_pts_per_vec * 2
            self.lane_uncertainty_proj = nn.Linear(feature_dim, self.embed_dims)

        # ROI uncertainty adapter (progressive ROI scale adjustment)
        self.roi_uncertainty_cfg = roi_uncertainty_cfg or {}
        if self.roi_uncertainty_cfg.get('use_roi_uncertainty', False):
            from ..modules.roi_uncertainty_adapter import ROIUncertaintyAdapter
            adapter_cfg = self.roi_uncertainty_cfg.get('adapter_cfg', {})
            self.roi_uncertainty_adapter = ROIUncertaintyAdapter(**adapter_cfg)
            self.init_roi_scale = self.roi_uncertainty_cfg.get('init_scale', 1.0)

        # ROI scale consistency loss (optional)
        if roi_scale_loss_cfg is not None:
            from ..losses import create_roi_scale_loss
            self.roi_scale_loss_fn = create_roi_scale_loss(roi_scale_loss_cfg)
    ...
    def _init_layers(self):
        ...
        if self.aux_seg['use_aux_seg']:
            ...
            if self.aux_seg['bev_seg']:
                ...
                # Wrap with uncertainty wrapper if enabled
                if self.use_segmentation_uncertainty:
                    from .segmentation_uncertainty_heads import create_uncertainty_head
                    seg_uncertainty_cfg = self.segmentation_uncertainty_cfg.get('bev_seg_cfg', {})
                    self.seg_head = create_uncertainty_head(seg_head_base, 'edge_cls', seg_uncertainty_cfg)
                else:
                    self.seg_head = seg_head_base
            if self.aux_seg['pv_seg']:            
                pv_seg_head_base = nn.Sequential(
                    nn.Conv2d(self.embed_dims, self.embed_dims, kernel_size=3, padding=1, bias=False),
                    # nn.BatchNorm2d(128),
                    nn.ReLU(inplace=True),
                    nn.Conv2d(self.embed_dims, self.aux_seg['seg_classes'], kernel_size=1, padding=0)
                ...
                if self.use_segmentation_uncertainty:
                    from .segmentation_uncertainty_heads import create_uncertainty_head
                    pv_uncertainty_cfg = self.segmentation_uncertainty_cfg.get('pv_seg_cfg', {})
                    self.pv_seg_head = create_uncertainty_head(pv_seg_head_base, 'edge_cls', pv_uncertainty_cfg)
                else:
                    self.pv_seg_head = pv_seg_head_base
    ...
    def forward(self, mlvl_feats, lidar_feat, img_metas, prev_bev=None,  only_bev=False):
        ...
        if only_bev:  # only use encoder to obtain BEV features, TODO: refine the workaround
            ...
            # First pass: get initial predictions (may use previous roi_scales if available)
            roi_scales_for_decoder = None
            if hasattr(self, 'roi_uncertainty_adapter') and hasattr(self, '_cached_roi_scales'):
                # Use cached roi_scales from previous iteration, but ensure shape matches current num_vec.
                # cached shape expected: (num_layers, bs, cached_num_vec, 1)
                cached = self._cached_roi_scales
                try:
                    cached_num_vec = cached.shape[2]
                except Exception:
                    cached_num_vec = None
                if cached_num_vec is None:
                    roi_scales_for_decoder = cached
                elif cached_num_vec != num_vec:
                    if cached_num_vec > num_vec:
                        # Trim cached scales to match current number of vectors
                        roi_scales_for_decoder = cached[:, :, :num_vec, :].contiguous()
                    else:
                        # Expand cached scales by repeating along vector dimension
                        reps = num_vec // cached_num_vec
                        rem = num_vec % cached_num_vec
                        roi_scales_for_decoder = cached.repeat(1, 1, reps, 1)
                        if rem > 0:
                            roi_scales_for_decoder = torch.cat(
                                [roi_scales_for_decoder, cached[:, :, :rem, :]], dim=2)
                else:
                    roi_scales_for_decoder = cached
            ...
            outputs = self.transformer(
                ...
                roi_scales=roi_scales_for_decoder,
        )
        ...
        roi_scales = []
        if hasattr(self, 'roi_uncertainty_adapter'):
            init_scale = getattr(self, 'init_roi_scale', 1.0)
            prev_scales = hs.new_full((bs, num_vec, 1), init_scale)
            roi_scales.append(prev_scales)
        for lvl in range(hs.shape[0]):
            ...
            # Progressive ROI scale estimation for next decoder layer
            if hasattr(self, 'roi_uncertainty_adapter'):
                next_scales = self.roi_uncertainty_adapter(
                    outputs_pts_coord, None, prev_scales)
                roi_scales.append(next_scales)
        ...
        outputs_seg = None
        outputs_pv_seg = None
        seg_uncertainty_info = {}
        pv_seg_uncertainty_info = {}
        if self.aux_seg['use_aux_seg']:
            seg_bev_embed = bev_embed.permute(1,0,2).view(bs,self.bev_h, self.bev_w, -1).permute(0,3,1,2).contiguous()
            if self.aux_seg['bev_seg']:
                if self.use_segmentation_uncertainty:
                    seg_output = self.seg_head(seg_bev_embed, return_uncertainty=True)
                    if isinstance(seg_output, dict):
                        outputs_seg = seg_output.get('mean_logits', seg_output.get('logits'))
                        seg_uncertainty_info = {k: v for k, v in seg_output.items() if k not in ['mean_logits', 'logits']}
                    else:
                        outputs_seg = seg_output
                else:
                    outputs_seg = self.seg_head(seg_bev_embed)
            bs, num_cam, embed_dims, feat_h, feat_w = mlvl_feats[-1].shape
            if self.aux_seg['pv_seg']:
                if self.use_segmentation_uncertainty:
                    pv_seg_output = self.pv_seg_head(mlvl_feats[-1].flatten(0,1), return_uncertainty=True)
                    if isinstance(pv_seg_output, dict):
                        outputs_pv_seg = pv_seg_output.get('mean_logits', pv_seg_output.get('logits'))
                        pv_seg_uncertainty_info = {k: v for k, v in pv_seg_output.items() if k not in ['mean_logits', 'logits']}
                    else:
                        outputs_pv_seg = pv_seg_output
        ...
        outs = {
            ...
            'seg_uncertainty': seg_uncertainty_info if seg_uncertainty_info else None,
            'pv_seg_uncertainty': pv_seg_uncertainty_info if pv_seg_uncertainty_info else None,
            "one2many_outs": dict(
        ...
        if roi_scales:
            roi_scales_tensor = torch.stack(roi_scales)  # (num_layers+1, bs, num_vec, 1)
            outs['roi_scales'] = roi_scales_tensor
            
            # Cache roi_scales for next iteration (for inference or next training step)
            if hasattr(self, 'roi_uncertainty_adapter'):
                # Remove first layer (initial scale) and use for next decoder pass
                self._cached_roi_scales = roi_scales_tensor[1:]  # (num_layers, bs, num_vec, 1)
            
            # Optional: Re-run decoder with computed roi_scales (only in training, if enabled)
            if (hasattr(self, 'roi_uncertainty_cfg') and 
                self.roi_uncertainty_cfg.get('use_roi_uncertainty', False) and
                self.roi_uncertainty_cfg.get('refine_with_roi_scales', False) and
                self.training and len(roi_scales) > 1):
                # Re-run transformer with computed roi_scales for refinement
                outputs_refined = self.transformer(
                    mlvl_feats,
                    lidar_feat,
                    bev_queries,
                    object_query_embeds,
                    ...
                    num_vec=num_vec,
                    num_pts_per_vec=self.num_pts_per_vec,
                    roi_scales=roi_scales_tensor[1:],  # Skip initial scale
                )
                
                # Use refined outputs
                bev_embed, depth, hs, init_reference, inter_references = outputs_refined
                hs = hs.permute(0, 2, 1, 3)
                
                # Re-compute outputs with refined features
                outputs_classes_one2one = []
                outputs_coords_one2one = []
                outputs_pts_coords_one2one = []
                outputs_classes_one2many = []
                ...
                for lvl in range(hs.shape[0]):
                    ...
                    outputs_pts_coords_one2one.append(outputs_pts_coord[:, 0:self.num_vec_one2one])
                    outputs_classes_one2many.append(outputs_class[:, self.num_vec_one2one:])
                    outputs_coords_one2many.append(outputs_coord[:, self.num_vec_one2many:])
                    outputs_pts_coords_one2many.append(outputs_pts_coord[:, self.num_vec_one2many:])
                ...
                # Update outs with refined predictions
                outs['all_cls_scores'] = outputs_classes_one2one
                outs['all_bbox_preds'] = outputs_coords_one2one
                outs['all_pts_preds'] = outputs_pts_coords_one2one
                outs['one2many_outs']['all_cls_scores'] = outputs_classes_one2many
                outs['one2many_outs']['all_bbox_preds'] = outputs_coords_one2many
                outs['one2many_outs']['all_pts_preds'] = outputs_pts_coords_one2many

        # Add lane uncertainty estimation if configured
        if hasattr(self, 'lane_uncertainty_head') and not only_bev:
            # Use the last decoder layer's class scores and pts predictions for uncertainty
            last_cls_scores = outputs_classes_one2one[-1]  # [bs, num_vec_one2one, num_classes]
            last_pts_preds = outputs_pts_coords_one2one[-1]  # [bs, num_vec_one2one, num_pts_per_vec, 2]

            # Flatten for uncertainty head input
            bs, num_vec, num_classes = last_cls_scores.shape
            cls_features = last_cls_scores.view(bs * num_vec, num_classes)  # [bs*num_vec, num_classes]
            pts_features = last_pts_preds.view(bs * num_vec, -1)  # [bs*num_vec, num_pts_per_vec*2]

            # Concatenate features
            lane_features = torch.cat([cls_features, pts_features], dim=-1)  # [bs*num_vec, feature_dim]

            # Project to embed_dims for the uncertainty head
            if hasattr(self, 'lane_uncertainty_proj'):
                lane_features = self.lane_uncertainty_proj(lane_features)

            # Get uncertainty estimation
            uncertainty_output = self.lane_uncertainty_head(
                lane_features, return_uncertainty=True
            )

            # Add uncertainty results to output
            outs.update({
                'lane_uncertainty_logits': uncertainty_output['logits'].view(bs, num_vec, num_classes),
                'lane_uncertainty_scores': uncertainty_output['uncertainty'].view(bs, num_vec),
                'lane_entropy': uncertainty_output['entropy'].view(bs, num_vec),
                'lane_all_logits': uncertainty_output['all_logits'],
                'positive_mask': uncertainty_output.get('positive_mask'),
                'negative_mask': uncertainty_output.get('negative_mask'),
                'uncertainty_scores': uncertainty_output.get('uncertainty_scores')
            })
    ...
    def loss(self,
        ...
        # ROI scale consistency loss（可选）
        if hasattr(self, 'roi_scale_loss_fn'):
            roi_scales = preds_dicts.get('roi_scales', None)
            if roi_scales is not None and roi_scales.shape[0] > 1:
                consistency_losses = []
                for i in range(1, roi_scales.shape[0]):
                    consistency_losses.append(
                        self.roi_scale_loss_fn(roi_scales[i - 1], roi_scales[i])
                    )
                if len(consistency_losses) > 0:
                    loss_dict['loss_roi_scale'] = torch.stack(consistency_losses).mean()

        # Add lane uncertainty loss if configured
        if hasattr(self, 'lane_uncertainty_loss_fn'):
            # Use the last decoder layer's predictions for uncertainty loss
            last_cls_scores = all_cls_scores[-1]  # [bs, num_vec_one2one, num_classes]
            bs, num_vec, num_classes = last_cls_scores.shape

            # Prepare lane features for uncertainty loss (similar to forward)
            cls_features = last_cls_scores.view(bs * num_vec, num_classes)
            last_pts_preds = all_pts_preds[-1]  # [bs, num_vec_one2one, num_pts_per_vec, 2]
            pts_features = last_pts_preds.view(bs * num_vec, -1)
            lane_features = torch.cat([cls_features, pts_features], dim=-1)

            # Get ground truth labels for uncertainty loss
            gt_labels_tensor = torch.stack(gt_labels_list, dim=0)  # [bs, num_gt]

            # Align targets shape with lane_features (bs*num_vec)
            if gt_labels_tensor.numel() == bs * num_vec:
                lane_targets = gt_labels_tensor.view(-1).to(lane_features.device)
            else:
                # Fallback: use predicted classes as pseudo targets to avoid shape mismatch
                lane_targets = cls_features.argmax(dim=-1)

            # Compute uncertainty loss
            uncertainty_loss_dict = self.lane_uncertainty_loss_fn(
                lane_features, lane_targets, preds_dicts
            )

            # Add uncertainty losses to loss dict
