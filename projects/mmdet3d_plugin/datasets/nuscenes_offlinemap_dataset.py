    ...
    def get_data_info(self, index):
        ...
        def convert_path(path):
            if path.startswith('./data/nuscenes/'):
                return path.replace('./data/nuscenes/', self.data_root)
            return path
        ...
        if 'lidar_path' in info:
            info['lidar_path'] = convert_path(info['lidar_path'])
        
        # 处理扫描路径
        if 'sweeps' in info:
            for sweep in info['sweeps']:
                if 'data_path' in sweep:
                    sweep['data_path'] = convert_path(sweep['data_path'])
                if 'lidar_path' in sweep:
                    sweep['lidar_path'] = convert_path(sweep['lidar_path'])
